...
async function searchCustomerByName(name) {
  try {
    ...
    const q = query(customersCollectionRef, where('company_associated', '==', companyemail), where(user_nameInput.value, '==', doc.data().user_fullname));
    const querySnapshot = await getDocs(q);

    querySnapshot.forEach((doc) => {
      console.log('Document ID:', doc.id);
      console.log('User Name:', doc.data().user_fullname);
      console.log('User Email:', doc.data().user_phone);
      console.log('User Phone:', doc.data().user_email);
    });
...
const searchInput = document.getElementById('search_customer'); // Assuming an input element with the ID 'search-input'
