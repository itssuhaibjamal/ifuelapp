...
// check if collection that we are fetching if its empty excute the else statement
   ...
   docSnap.forEach(doc => {
      ...
      if(doc.data().company_associated == companyemail){
        ...
        <td class='d-flex'>
        <a href='update-loan.html?update=${doc.id}' class='btn btn-success'>Update</a> &numsp;
        <a href='view-loans.html?delete=${doc.id}' class='btn btn-danger'>Delete</a></td>
        </tr>
    `;
      }else{
